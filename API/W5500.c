...
* 描述    : W5500 GPIO初始化配置 主要是RST和INT引脚 而CS引脚放在SPI中统一进行管理
* 输入    : 无
* 输出    : 无
* 返回值  : 无
* 说明    : 无
...
{
	SPIByte(SPIz,dat>>8);//写数据高位
	SPIByte(SPIz,dat);	//写数据低位
}
...
void Write_W5500_1Byte(unsigned short reg, unsigned char dat)
{
	...
	SPIByte(SPIz,FDM1|RWB_WRITE|COMMON_R);//通过SPI1写控制字节,1个字节数据长度,写数据,选择通用寄存器
	SPIByte(SPIz,dat);//写1个字节数据
...
void Write_W5500_2Byte(unsigned short reg, unsigned short dat)
{
	...
	SPI_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPIByte(SPIz,FDM2|RWB_WRITE|COMMON_R);//通过SPI1写控制字节,2个字节数据长度,写数据,选择通用寄存器
...
void Write_W5500_nByte(unsigned short reg, unsigned char *dat_ptr, unsigned short size)
{
	...
	SPI_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPIByte(SPIz,VDM|RWB_WRITE|COMMON_R);//通过SPI1写控制字节,N个字节数据长度,写数据,选择通用寄存器

	for(i=0;i<size;i++)//循环将缓冲区的size个字节数据写入W5500
	{
		SPIByte(SPIz,*dat_ptr++);//写一个字节数据
	}
...
void Write_W5500_SOCK_1Byte(SOCKET s, unsigned short reg, unsigned char dat)
{
	...
	SPI_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPIByte(SPIz,FDM1|RWB_WRITE|(s*0x20+0x08));//通过SPI1写控制字节,1个字节数据长度,写数据,选择端口s的寄存器
	SPIByte(SPIz,dat);//写1个字节数据
...
void Write_W5500_SOCK_2Byte(SOCKET s, unsigned short reg, unsigned short dat)
{
	...
	SPI_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPIByte(SPIz,FDM2|RWB_WRITE|(s*0x20+0x08));//通过SPI1写控制字节,2个字节数据长度,写数据,选择端口s的寄存器
	SPI_Send_Short(dat);//写16位数据
...
void Write_W5500_SOCK_4Byte(SOCKET s, unsigned short reg, unsigned char *dat_ptr)
{
	...
	SPI_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPIByte(SPIz,FDM4|RWB_WRITE|(s*0x20+0x08));//通过SPI1写控制字节,4个字节数据长度,写数据,选择端口s的寄存器

	SPIByte(SPIz,*dat_ptr++);//写第1个字节数据
	SPIByte(SPIz,*dat_ptr++);//写第2个字节数据
	SPIByte(SPIz,*dat_ptr++);//写第3个字节数据
	SPIByte(SPIz,*dat_ptr++);//写第4个字节数据
...
unsigned char Read_W5500_1Byte(unsigned short reg)
{
	...
	SPI_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPIByte(SPIz,FDM1|RWB_READ|COMMON_R);//通过SPI1写控制字节,1个字节数据长度,读数据,选择通用寄存器

	//i=SPI_I2S_ReceiveData(SPIz);
	i = SPIByte(SPIz,0x00);//发送一个哑数据
	//i=SPI_I2S_ReceiveData(SPIz);//读取1个字节数据
...
unsigned char Read_W5500_SOCK_1Byte(SOCKET s, unsigned short reg)
{
	...
	SPI_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPIByte(SPIz,FDM1|RWB_READ|(s*0x20+0x08));//通过SPI1写控制字节,1个字节数据长度,读数据,选择端口s的寄存器
...
unsigned short Read_W5500_SOCK_2Byte(SOCKET s, unsigned short reg)
{
	...
	SPI_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPIByte(SPIz,FDM2|RWB_READ|(s*0x20+0x08));//通过SPI1写控制字节,2个字节数据长度,读数据,选择端口s的寄存器

	//i=SPI_I2S_ReceiveData(SPIz);
	i=SPIByte(SPIz,0x00);//发送一个哑数据
	//i=SPI_I2S_ReceiveData(SPIz);//读取高位数据
	i*=256;
	i+=SPIByte(SPIz,0x00);//发送一个哑数据
...
unsigned short Read_SOCK_Data_Buffer(SOCKET s, unsigned char *dat_ptr)
{
	...
	j=SPIByte(SPIz,VDM|RWB_READ|(s*0x20+0x18));//写控制字节,N个字节数据长度,读数据,选择端口s的寄存器
	//j=SPI_I2S_ReceiveData(SPIz);
	
	if((offset+rx_size)<S_RX_SIZE)//如果最大地址未超过W5500接收缓冲区寄存器的最大地址
	{
		for(i=0;i<rx_size;i++)//循环读取rx_size个字节数据
		{
			j=SPIByte(SPIz,0x00);//发送一个哑数据
			//j=SPI_I2S_ReceiveData(SPIz);//读取1个字节数据
...
void Write_SOCK_Data_Buffer(SOCKET s, unsigned char *dat_ptr, unsigned short size)
{
	...
	SPI_Send_Short(offset);//写16位地址
	SPIByte(SPIz,VDM|RWB_WRITE|(s*0x20+0x10));//写控制字节,N个字节数据长度,写数据,选择端口s的寄存器

	if((offset+size)<S_TX_SIZE)//如果最大地址未超过W5500发送缓冲区寄存器的最大地址
	{
		for(i=0;i<size;i++)//循环写入size个字节数据
		{
			SPIByte(SPIz,*dat_ptr++);//写入一个字节的数据		
		}
	}
...
void W5500_Hardware_Reset(void)
{
	...
	while(vW5500TimerGetValue() < 50);
	
	GPIO_SetBits(W5500_RST_PORT, W5500_RST);//复位引脚拉高
	vW5500TimerSetValue(0);
	while(vW5500TimerGetValue() < 200);

	/*路由必须上电 网络接口必须连接 此条语句才能连接*/
...
unsigned char Detect_Gateway(void)
{
	...
	vW5500TimerSetValue(0);
	while(vW5500TimerGetValue() < 5);//延时5ms 	
...
* 描述    : W5500中断处理程序框架 通过这个函数不停扫描W5500的状态，然后映射到
						变量S0_State中。
* 输入    : 无
* 输出    : 无
* 返回值  : 无
...
void W5500_Interrupt_Process(void)
{
	...
	i=Read_W5500_1Byte(SIR);			/*读取端口中断标志寄存器 用来判断当前状态*/
	
	if((i & S0_INT) == S0_INT)		/*Socket0事件处理 就是有socket0 事件发生了*/
	{
		j=Read_W5500_SOCK_1Byte(0,Sn_IR);//读取Socket0中断标志寄存器
		Write_W5500_SOCK_1Byte(0,Sn_IR,j);
		
		/*TCP模式下连接状态的判断，如果都不能连接 其他操作也没有意义*/
		if(j&IR_CON)				/*在TCP模式下,Socket0成功连接 只有连接上才可以进行收发*/
		{
